...
if (!function_exists('debug_log')) {
    function debug_log($message, $data = null) {
        ...
        // Chemin vers le fichier de log sur le serveur de production
        $logPath = 'https://api.rando.partageonslaforet.be/logs/test.log';
        
        // Utiliser file_put_contents avec le contexte HTTP
        $options = array(
            'http' => array(
                'method'  => 'POST',
                'header'  => 'Content-type: application/x-www-form-urlencoded',
                'content' => http_build_query(array('log' => $log))
            )
        );
        $context = stream_context_create($options);
        file_put_contents($logPath, $log, FILE_APPEND, $context);
    }
}
    ...
    try {
        ...
        $mail->Body = "
            <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
                ...
                <p>Si vous n'avez pas créé de compte sur PartagonsLaForet, vous pouvez ignorer cet email.</p>
                <p style='margin-top: 30px;'>
                    Cordialement,<br>
                    L'équipe PartagonsLaForet
                </p>
            </div>
        ";
        $mail->AltBody = "
